...
## Task 4: 日志框架配置 (structlog)

> [!IMPORTANT]
> 本项目使用 **structlog** 作为**唯一**日志框架。所有模块**必须**通过 `structlog.get_logger()` 获取 logger，**严禁**使用 Python 标准库 `logging.getLogger()` 或 `print()` 进行日志输出。此约定贯穿整个项目生命周期。

**Files:**
- Create: `gmp/core/logging.py`
...
def setup_logging(log_level: str = "INFO") -> None:
    """配置 structlog processors 链 — 参见设计文档 08-operations.md §8.5

    本项目唯一日志框架: structlog
    所有后续模块中获取 logger 的方式:
        import structlog
    ...
    structlog.configure(
        processors=[
            ...
            structlog.dev.ConsoleRenderer(),  # 开发模式
        ],
...
| ✅ 正确 | ❌ 禁止 |
|---------|---------|
| `import structlog; logger = structlog.get_logger()` | `import logging; logger = logging.getLogger()` |
| `logger.info("event", key=value)` | `print("debug info")` |

**Step 1: 创建 `gmp/core/logging.py`**

**Step 2: 在 `gmp/main.py` 的 CLI 入口调用 `setup_logging(config.log_level)`**

**Step 3: Commit**

```bash
git add -A && git commit -m "chore(M01): add structlog logging configuration"
```
