...
> [!WARNING]
> **MeteoFetcher 缓存行为待修复**: `fetch_hourly` 设计意图为一次性返回完整 `days` 天 DataFrame，Scheduler 拿到后按日切片构建 DataContext。但当前 M06 实现中缓存命中时仅查询 `today` 单日，返回的是单日子集而非完整多日数据。在实现 Scheduler 前需修复 `fetch_hourly` 的缓存读取逻辑：应查询所有 `days` 天的缓存，全部命中时拼合返回完整 DataFrame；任一天未命中则调用 API。
